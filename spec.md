...
- **静的解析ベース**: 構文木（Python: AST、Java: CST）に基づく解析を行い、正規表現による簡易解析は避ける
...
**ファイル名のサニタイズ**:
- ファイル名に使用できない文字（`< > : " / \ | ? *`）はファイル名生成時に除去する
- これにより、Windows 環境でのファイルシステム互換性を確保する
- 例：Java コンストラクタ `<init>` → `User_init.java`（`<>` が除去される）
- サニタイズはシンボルの論理名（`name`、`qualname`、`display_name()`）には影響しない。INDEX.md 内のシンボル表示名や parts ファイル内のヘッダコメントでは元の名前（`User#<init>` 等）がそのまま使用される

**名前衝突の回避**:
- Java のメソッドオーバーロード等で同名シンボルが存在する場合、シグネチャまたは表示名の SHA-256 ハッシュ先頭 4 文字を付与する
- サニタイズにより同名になった場合も同様にハッシュが付与される（例：メソッド `init()` とコンストラクタ `<init>` が同一クラスに存在する場合）
...
言語判定（自動検出 or 明示指定）
    ↓
パーサー選択（Python AST / Java tree-sitter）
    ↓
ソースコード解析 → シンボルリスト + 警告
    ↓
...
パーサーはソースコードを構文木に変換し、以下の情報を持つシンボルオブジェクトを抽出する。
...
**解析基盤**: `tree-sitter` + `tree-sitter-java` ライブラリ（CST ベース）
...
| Java 8+ 構文（ラムダ、メソッドリファレンス等） | 対応 |

**メタデータ抽出**:
- Javadoc：シンボルノードの直前に位置する `/** ... */` コメントを CST の兄弟ノードから取得、最初の文（ピリオドまたは改行まで）を role として使用
- 呼び出し関係：`method_invocation` ノードを再帰的に走査して抽出
- 依存関係：`import_declaration` ノードを解析

**行範囲の決定**:
- tree-sitter が各ノードの開始・終了位置（行・列）を直接提供するため、ブレースマッチング等の後処理は不要

**パースエラー処理**:
- `root_node.has_error` でエラーを検出
- エラーノードの行・列番号を含む警告メッセージを返す
- エラーがあっても抽出済みのシンボルは返却（部分的な結果を維持）
...
- Java は tree-sitter ベースのため、新言語も tree-sitter 対応パーサーを優先して採用する
