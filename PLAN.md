...
| APIエラー（5xx） | API応答コード | 「サーバーエラーが発生しました。再試行してください」 | 自動リトライ（最大3回）後、手動再実行を促す |
| タイムアウト | 応答時間 > 60秒 | 「処理がタイムアウトしました。再試行してください」 | 章単位で分割実行を促す |
...
| 形式不正 | Markdownパース失敗 | 「出力形式が不正です。再試行してください」 | 自動リトライ後、手動再実行を促す |
