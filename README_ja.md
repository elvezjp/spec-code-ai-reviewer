...
**本ツールはローカル実行を前提としており、アプリケーション自体に認証・認可の仕組みを持ちません。**

全 API エンドポイントは認証なしで呼び出せます。到達できる相手は、LLM を無制限に呼び出せるほか、アップロードされた設計書やコードを読み取れます。

そのため、**ネットワークには公開しないでください。** バックエンドは `127.0.0.1` にバインドして起動します（「[起動](#起動)」参照）。

意図してネットワークに公開する場合は、**前段に認証を置いてください。** リバースプロキシで Basic 認証をかける構成例を [docs/ec2-deployment-spec.md](docs/ec2-deployment-spec.md) に記載しています（nginx の `auth_basic`）。アプリケーション側に認証を実装しない方針は [Issue #134](https://github.com/elvezjp/spec-code-ai-reviewer/issues/134) で選択肢 A として決定したものです。

詳細は [SECURITY_ja.md](SECURITY_ja.md) を参照してください。
...
- **必須バージョン**: Node.js 20.19以上（20.x）、22.12以上（22.x）、または24以上
- **推奨バージョン**: Node.js 22.12以上（22.x）
...
**注意**: `--host 127.0.0.1` を明示しています。この API は認証なしで公開されるため、`0.0.0.0` でバインドすると同一ネットワーク上の誰でも全エンドポイントを呼び出せます。意図してネットワークに公開する場合を除き、この指定を変更しないでください。
...
| 変数名 | 説明 | デフォルト値 |
|--------|------|-------------|
| `CORS_ORIGINS` | ブラウザからのクロスオリジンアクセスを許可するオリジン（カンマ区切り） | ローカル開発用オリジンのみ |

未設定時は Vite の開発サーバー／プレビュー（`http://localhost:5173`、
`http://127.0.0.1:5173`、`http://localhost:4173`、`http://127.0.0.1:4173`）
のみを許可します。CORS はブラウザによるクロスオリジン応答の読み取りを
制御するもので、認証やネットワークアクセス制限の代わりにはなりません。

本番環境では、フロントエンドを配信するオリジンを明示してください。

```bash
CORS_ORIGINS=https://example.com
```

`*`（全許可）も指定できますが、その場合は認証情報の送受信を無効化します。
任意のサイトから応答を読めるようになるため、推奨しません。

**補足**: 通常の構成でこの変数を設定する必要はありません。開発時はフロント
エンドからの `/api` リクエストを Vite の proxy がサーバー側で中継し、本番は
同じ FastAPI アプリがフロントエンドを配信するため、いずれもブラウザから見て
同一オリジンであり CORS は発生しません。フロントエンドを別ホストで動かす
構成の場合のみ設定します。
