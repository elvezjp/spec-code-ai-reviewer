...
7. SettingsModal コンポーネント（共通設定モーダル）
   - プログラム情報表示セクション
...
8. TokenEstimator コンポーネント
   - 推定トークン数の計算・表示
   - 入力テキストの監視

9. SystemPromptEditor コンポーネント
   - プリセット選択
   - 役割、目的、出力形式、注意事項の編集
   - プレビュー機能

10. ScreenContainer コンポーネント（画面切り替え管理）
...
| C-06 | TextInputが入力を受け付けること | 2 |
| C-07 | TextAreaが入力を受け付けること | 2 |
| C-08 | Selectが選択を受け付けること | 2 |
| C-09 | RadioGroupが選択を受け付けること | 2 |
| C-10 | FileInputがファイル選択を受け付けること | 2 |
| C-11 | Modalの開閉が正しく動作すること | 2 |

### Coreコンポーネント（共通機能）

| ID | 項目 | Phase |
|----|------|-------|
| C-12 | SettingsModalが開閉できること | 2 |
| C-13 | SettingsModalで各セクションの表示/非表示が制御できること | 2 |
| C-14 | SettingsModalに拡張セクションを追加できること | 2 |
| C-15 | ProgramInfoSectionがパラメータに応じた情報を表示すること | 2 |
| C-16 | ConfigFileSectionで設定ファイルのアップロード/保存ができること | 2 |
| C-17 | LlmSettingsSectionでプロバイダー選択・API設定ができること | 2 |
| C-18 | PromptPresetsSectionでプリセット管理ができること | 2 |
| C-19 | ScreenContainerでmain画面が表示されること | 2 |
| C-20 | ScreenContainerでexecuting画面に切り替わること | 2 |
| C-21 | ScreenContainerでresult画面に切り替わること | 2 |
| C-22 | TokenEstimatorが推定トークン数を表示すること | 2 |
| C-23 | SystemPromptEditorでプリセット選択・編集ができること | 2 |
