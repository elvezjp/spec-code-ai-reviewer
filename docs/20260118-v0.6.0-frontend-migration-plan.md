...
| VersionSelector | バージョン選択ドロップダウン（ピル型） |
...
**試験項目**:
- [x] `npm run dev` でVite開発サーバーが起動すること
- [x] `npm run build` でビルドが成功すること
- [x] Tailwind CSSのスタイルが適用されること
- [x] TypeScriptのコンパイルエラーがないこと
...
12. VersionSelector コンポーネント
    - ピル型のバージョン選択ボタン
    - バルーン形式のドロップダウン
    - 現在のバージョン表示、バージョン一覧から選択
...
**試験項目**:
- [x] Layoutコンポーネントが正しくレンダリングされること
- [x] Headerコンポーネントがタイトル、設定ボタンを表示すること
- [x] Cardコンポーネントが正しいスタイルで表示されること
- [x] Buttonコンポーネントの各バリアント（Primary/Secondary/Danger）が正しく表示されること
- [x] Buttonのdisabled状態が視覚的に反映されること
- [x] FileInputButtonでファイル選択ができること
- [x] FileDropzoneでドラッグ&ドロップができること
- [x] Modalの開閉が正しく動作すること
- [x] SettingsModalが開閉できること
- [x] SettingsModalで各セクションの表示/非表示が制御できること
- [x] SettingsModalに拡張セクションを追加できること
- [x] ScreenContainerで画面切り替えが正しく動作すること
- [x] TokenEstimatorが推定トークン数を表示すること
- [x] SystemPromptEditorでプリセット選択・編集ができること
- [x] VersionSelectorでバージョン一覧が表示されること
- [x] VersionSelectorでバージョン選択ができること
...
│   ├── SpecFileList.tsx              # 設計書ファイル一覧・優先度設定
│   ├── CodeFileList.tsx              # プログラムファイル一覧
│   ├── ConversionPreview.tsx         # 変換結果プレビュー
│   ├── ReviewRunner.tsx              # レビュー実行ボタン・状態表示
│   └── ReviewResult.tsx              # レビュー結果表示（タブ切り替え含む）
├── hooks/
│   ├── useReviewerState.ts
...
**試験項目**:
- [x] 設定モーダルが開閉できること
- [x] 設定モーダルに設計書種別セクションが表示されること
- [x] API URL、モデル選択が保存されること
- [x] Excelファイルをアップロードできること
- [x] 複数の設計書ファイルの優先度を設定できること
- [x] 設計書種別を選択できること
- [x] プログラムファイルをアップロードできること
- [x] マークダウン変換が実行できること
- [x] 変換結果がプレビュー表示されること
- [x] 推定トークン数が表示されること
- [x] システムプロンプトのプリセットを選択できること
- [x] システムプロンプトの各項目を編集できること
- [x] レビューが実行できること
- [x] レビュー結果がストリーミング表示されること
- [x] ZIP出力が正常に動作すること

**v0.5.2との比較確認**:

Phase 3完了時に、v0.5.2のAIレビュアー（`versions/v0.5.2/frontend/index.html`）と比較して以下を確認する。

| 確認項目 | 確認内容 |
|----------|----------|
| 画面構成 | ヘッダー、ファイルアップロード、システムプロンプト設定、実行ボタン、結果画面の各セクションが揃っているか |
| 設定モーダル | プログラム情報、設定ファイル、LLM設定、プロンプトプリセット、設計書種別、変換ツールの全セクションがあるか |
| ファイル操作 | 設計書・プログラムのアップロード、優先度設定、種別選択、変換、プレビュー、ダウンロードが動作するか |
| レビュー機能 | 2回レビュー実行、タブ切り替え、簡易判定、詳細レポート、一式ダウンロード（ZIP）が動作するか |
| 表示テキスト | ラベル、説明文、注意書き、リンク等が漏れなく移行されているか |
| スタイル | 色、レイアウト、余白、アニメーション等がv0.5.2と同等か |
...
**試験項目**:
- [x] infoセクションが固定値と自動生成値を表示すること
- [x] LLMプロバイダーを切り替えられること
- [x] プロバイダー切替時にフィールドが動的に変更されること
- [x] Bedrockプロバイダー選択時に注意事項が表示されること
- [x] API Key等の必須項目が未入力の場合エラーが表示されること
- [x] モデル配列に項目を追加・削除できること
- [x] specTypesテーブルに行を追加・削除できること
- [x] systemPromptsセクションを追加・削除できること
- [x] systemPromptsセクションの折りたたみが動作すること
- [x] ダウンロードボタンで設定ファイル（.md）がダウンロードされること
- [x] 生成されたマークダウンが正しいフォーマットであること

**v0.5.2との比較確認**:

Phase 4完了時に、v0.5.2の設定ファイルジェネレーター（`versions/v0.5.2/frontend/config-file-generator/index.html`）と比較して以下を確認する。

| 確認項目 | 確認内容 |
|----------|----------|
| 画面構成 | ヘッダー、各セクション（info、llm、specTypes、systemPrompts）、ダウンロードボタンが揃っているか |
| infoセクション | プログラム名、バージョン、生成日時が正しく表示・生成されるか |
| llmセクション | プロバイダー切替、各プロバイダー固有フィールド、モデル配列の追加・削除が動作するか |
| specTypesセクション | テーブル形式での行追加・削除・編集が動作するか |
| systemPromptsセクション | セクション追加・削除、折りたたみ、各フィールド編集が動作するか |
| 表示テキスト | ラベル、説明文、プレースホルダー、注意書きが漏れなく移行されているか |
| 出力ファイル | 生成されるマークダウンの形式がv0.5.2と同一か |
...
**試験項目**:
- [x] `/` でAIレビュアーページが表示されること
- [x] `/config-generator` で設定ファイルジェネレーターページが表示されること
- [x] 存在しないパスで404ページが表示されること

---

### Phase 6: アイコン統一（lucide-react導入）

**目的**: 絵文字をlucide-reactアイコンに置き換え、UIの一貫性を向上

**作業内容**:
1. lucide-reactパッケージのインストール
2. 絵文字をlucide-reactアイコンに置き換え

**対象ファイルと置き換え内容**:

| ファイル | 絵文字 | 置き換えアイコン |
|----------|--------|------------------|
| `core/components/ui/Modal.tsx` | × | X |
| `core/components/shared/SettingsModal/SettingsModal.tsx` | ⚙️ | Settings |
| `core/components/shared/TokenEstimator.tsx` | ⚠️ | AlertTriangle |
| `core/components/shared/SystemPromptEditor.tsx` | ▼ | ChevronDown / ChevronRight |
| `features/reviewer/index.tsx` | ⚙️ | Settings |
| `features/reviewer/index.tsx` | 📄 | FileText |
| `features/reviewer/components/CodeFileList.tsx` | 📥 | Download |
| `features/reviewer/components/CodeFileList.tsx` | ▼▶ | ChevronDown / ChevronRight |
| `features/reviewer/components/SpecFileList.tsx` | 📥 | Download |
| `features/reviewer/components/SpecFileList.tsx` | ▼▶ | ChevronDown / ChevronRight |
| `features/reviewer/components/ReviewResult.tsx` | ⚠️ | AlertTriangle |
| `features/reviewer/components/ReviewResult.tsx` | 📥 | Download |
| `features/config-generator/components/DownloadButton.tsx` | 📥 | Download |
| `features/config-generator/components/SpecTypesTable.tsx` | × | X |
| `features/config-generator/components/SystemPromptsSection.tsx` | ▼▶ | ChevronDown / ChevronRight |
| `features/config-generator/components/SystemPromptsSection.tsx` | × | X |
| `features/config-generator/components/LlmSection.tsx` | ⚠ | AlertTriangle |
| `features/config-generator/components/DynamicFieldArray.tsx` | × | X |

**試験項目**:
- [x] lucide-reactがインストールされていること
- [x] 全ての絵文字がアイコンに置き換えられていること
- [x] アイコンが正しく表示されること
- [x] ビルドが成功すること
...
| C-21 | VersionSelectorでバージョン一覧が表示されること | 2 | |
| C-22 | VersionSelectorでバージョン選択ができること | 2 | |
...
| I-02 | `/config-file-generator` で設定ファイルジェネレーターページが表示されること | 5 | |
...
| ID | 項目 | Phase | 結果 |
|----|------|-------|------|
| A-01 | lucide-reactがインストールされていること | 6 | |
| A-02 | 全ての絵文字がアイコンに置き換えられていること | 6 | |
| A-03 | アイコンが正しく表示されること | 6 | |
| A-04 | ビルドが成功すること | 6 | |
...
| ID | 項目 | Phase | 結果 |
|----|------|-------|------|
| Q-01 | 全ての単体テストがパスすること | 7 | |
| Q-02 | 既存のv0.5.2と同等の機能が動作すること | 7 | |
...
| 2026-01-18 | reviewerコンポーネント見直し: VersionSelectorをsharedに移動、FileUploaderを削除（FileInputButton/FileDropzoneで代替） |
| 2026-01-18 | Phase 3・Phase 4にv0.5.2との比較確認作業を追加 |
| 2026-01-18 | Phase 6（アイコン統一）を追加、既存Phase 6をPhase 7に変更 |
| 2026-01-18 | Phase 7からE2Eテスト・Lighthouseテストを除外、単体テストとv0.5.2機能同等性確認に集中 |
