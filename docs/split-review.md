...
- 設計書パーツテーブルまたはコードパーツテーブルで各パートの「要約」チェックボックスを選択し、プレビュー結果末尾の「選択した要約を実行」ボタンで一括実行（設計書→コードの順に逐次実行）
...
結果統合でエラーが発生した場合、各グループのレビュー結果を個別に「そのまま」「要約」「スキップ」の3択で切り替えてリトライできます。

- **スキップ**: そのグループのレビュー結果を統合対象から除外する。グループ数が多くトークン上限を超える場合に、重要度の低いグループをスキップすることで要約の実行回数を減らしつつトークン消費を効率的に削減できる
- スキップしたグループは最終レポートに含まれないが、一式ダウンロード（ZIP）のグループレビュー個別ファイルには含まれる
...
```
ユーザー: 大規模な設計書.xlsx（または.docx） + UserService.java をアップロード
    ↓
[Excel/Word→Markdown変換、コードに行番号付与]（既存機能）
    ↓ ユーザーが「分割」モードを選択
