...
**ファイル**:
- [presetCatalog.ts](../versions/v0.9.0/frontend/src/core/data/presetCatalog.ts) — モード別プリセット定義（systemPrompt の role / purpose / format / notes）
- [prompt_builder.py](../versions/v0.9.0/backend/app/services/prompt_builder.py) — 一括レビュー時のユーザープロンプト構築、構造マップセクション生成
- [review.py](../versions/v0.9.0/backend/app/routers/review.py) — 分割レビュー時のシステムプロンプト定数、各フェーズのプロンプト構築

### 4.1 標準プリセットの比較

`standard-review`（突合用）と `standard-mapping`（マッピング用）の主な差異は以下の通りです。

| 要素 | standard-review | standard-mapping |
|------|-----------------|------------------|
| format（出力形式） | **Markdown形式**（サマリー、突合結果一覧テーブル、詳細） | **JSON形式**（`files[].items[]` 構造）。サマリーは不要と明示し、itemsから自動集計する前提 |
| notes（判定基準） | **OK / NG / 要確認** の3段階判定 | **確信度 高 / 中 / 低** の3段階判定 |
| purpose（観点） | 整合性検証 — 差異・問題点の発見と報告 | マッピング — 設計項目ごとの実装箇所・実装要素・確信度の特定 |
| role | 突合し、整合性を検証するレビュアー | マッピングを行う専門家 |

最も大きな差異は **出力形式** です。突合モードはMarkdownレポートを期待するのに対し、マッピングモードは構造化されたJSONを期待します。この指定はシステムプロンプトの `format` フィールドで行われるため、ユーザープロンプト側の処理は共通です。

### 4.2 一括レビュー時（`/api/review`）

ユーザープロンプトがモードに応じて変わります。
...
出力形式の指定（Markdown / JSON）はユーザープロンプトではなく、**プリセットのシステムプロンプト**（`format` フィールド）で制御されます。

**構造マップセクションの形式**:

マッピングモードの場合（正確には`useStructureMap=true`の場合）、ユーザープロンプトの末尾に以下の形式で付加されます。
md2map、code2mapによって生成された`MAP.json`のうち、レビューに活用できるフィールドを抽出して、JSON形式でまとめたものです。
...
分割レビューの実行ロジックも **両モードで共通** です。
システムプロンプトの4要素から値を取得し、各フェーズのプロンプトに含めます。
システムプロンプトが未指定の場合のみフォールバック定数のテキストを利用します。
フォールバックのテキスト内容はモード別に異なりますが、基本的には利用されません。
...
- マッピングモードの場合（正確には`useStructureMap=true`の場合）、一括レビュー時と同様に、ユーザープロンプトの末尾に構造マップが追加されます。

**フェーズ3: 結果統合（`POST /api/review/integrate`）**

- フェーズ2と同様に、`useStructureMap` フラグによるり構造マップが追加されます。

#### フォールバック定数一覧（参考）

`systemPrompt` が指定されていない場合に使用されるフォールバックテキストの概要です。
すべてプリセットのシステムプロンプトに含まれるため、基本的に使用されることはありません。

| フェーズ | 要素 | 突合モード | マッピングモード |
|---------|------|-----------|----------------|
| 1 構造マッチング | role | 構造を分析する専門家 | マッピングを行う専門家 |
| 1 構造マッチング | purpose | 関連性の高いグループにまとめる | 実装箇所を推定してグループにまとめる |
| 2 グループレビュー | role | 整合性をレビューする専門家 | マッピングを行う専門家 |
| 2 グループレビュー | purpose | 整合性を確認し指摘事項を報告 | 実装箇所を特定 |
| 2 グループレビュー | format | サマリー、突合結果一覧、詳細 | マッピングサマリー、マッピング一覧、未マッピング項目 |
| 3 結果統合 | role | レビュー結果を統合するエキスパート | マッピング結果を統合するエキスパート |
| 3 結果統合 | purpose | Markdown形式のレビューレポートを生成 | Markdown形式のマッピングレポートを生成 |
| 3 結果統合 | format | Markdown形式のレビューレポート | Markdown形式のマッピングレポート + カバレッジ率 |

### 4.4 ユーザープロンプトの合成

突合モード、、マッピングモード、**両モードで共通** です。
...
| Method | Path | モード対応 |
|--------|------|-----------|
| POST | `/api/review` | `mode` に応じたユーザープロンプト構築。`useStructureMap` + `structureMap` で構造マップをプロンプトに組み込み |
...
[Phase 3] 統合
    ...
    → AIがJSON形式でマッピング結果を返却
    ↓
フロントエンドで JSON をパース → マッピング結果表示（一括実行と同じUI）
```
