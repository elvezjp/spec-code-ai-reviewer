...
設計書・コードそれぞれの「そのまま/要約」選択、要約実行ボタン、要約結果プレビューを管理するコンポーネント。
...
- **ユーザーへの注意表示**（SummarizePanelに表示する警告文）:
  ```
  ⚠️ 要約によりレビュー精度が低下する場合があります。
  - 数値条件や境界値の指摘が減少する可能性があります
  - エラーケースの漏れが検出されにくくなります
  - 要約結果をプレビューで確認してからリトライしてください
  ```
