...
エラー内容の解析は行わない。リトライ設定パネルはエラー種別（errorCode）に関わらず常に表示する（トークン超過以外のエラーでも、要約により入力量を削減してリトライが成功する可能性があるため）。
...
### I. ターミナル・バックエンド確認（コードレビュー・API呼び出し・ログで確認）

#### A. 施策1: MAP.json除外のコード確認

- [ ] `review.py` の Phase 2 全体構造コンテキスト構築で `documentMapJson`/`codeMapJson` のブロックが削除されていること
- [ ] `review.py` の Phase 3 全体構造コンテキスト構築で `documentMapJson`/`codeMapJson` のブロックが削除されていること
- [ ] `index.tsx` の `executeGroupReview` 呼び出しから `documentMapJson`/`codeMapJson` が除外されていること
- [ ] `index.tsx` の `executeIntegrate` 呼び出しから `documentMapJson`/`codeMapJson` が除外されていること
- [ ] Phase 1（構造マッチング）の `executeStructureMatching` 呼び出しは変更されていないこと
- [ ] Phase 2 で INDEX.md と全グループ一覧は引き続き送信されていること（コード確認）

#### B. 施策2: トークン超過判定関数のテスト

- [ ] `_is_token_limit_error("prompt is too long: 150000 tokens")` が `True` を返すこと（Anthropic形式）
- [ ] `_is_token_limit_error("maximum context length exceeded")` が `True` を返すこと（OpenAI形式）
- [ ] `_is_token_limit_error("Input is too long for requested model")` が `True` を返すこと（Bedrock形式）
- [ ] `_is_token_limit_error("Connection refused")` が `False` を返すこと（非トークンエラー）
- [ ] `_is_token_limit_error("Invalid API key")` が `False` を返すこと（認証エラー）
- [ ] `GroupReviewResponse` スキーマに `errorCode` フィールドが存在すること
- [ ] `IntegrateResponse` スキーマに `errorCode` フィールドが存在すること

#### C. 施策3: 要約APIのエンドポイント確認

- [ ] `POST /api/review/summarize` に `targetType: "design"` でリクエストして正常にレスポンスが返ること
- [ ] `POST /api/review/summarize` に `targetType: "code"` でリクエストして正常にレスポンスが返ること
- [ ] `POST /api/review/summarize` に `targetType: "review_result"` でリクエストして正常にレスポンスが返ること
- [ ] レスポンスに `originalTokens` / `summarizedTokens` が含まれ、正の整数であること
- [ ] レスポンスの `tokensUsed` に `input` / `output` が含まれること
- [ ] `llmConfig` で指定した provider/model が使用されること（ログまたはレスポンスで確認）
- [ ] 不正なリクエスト（空テキスト等）でもエラーレスポンス（`success: false`）が返り、500エラーにならないこと

### II. ブラウザ操作確認（UIで確認）

#### D. 施策1: MAP.json除外の動作確認

- [ ] 分割レビューの正常フロー（エラーなし）が従来通り完了すること
- [ ] MAP.json除外前後で、同一入力のレビュー結果に大きな品質差がないこと（主観評価）

#### E. 施策4: Phase 2（グループレビュー）の要約リトライUI

- [ ] グループレビューでエラー発生時、エラー内容（APIエラーメッセージ）がそのまま表示されること
- [ ] エラー発生時、エラー種別に関わらずリトライ設定パネルが表示されること（「そのまま/要約」のラジオボタン）
- [ ] エラー発生時、注意文（「⚠ 注意: 要約によって微妙なニュアンスや〜」）が表示されること
- [ ] 設計書/コードの「そのまま」ラジオボタンに推定トークン数が表示されること
- [ ] 「要約」を選択すると「選択した要約を実行」ボタンが表示されること
- [ ] 「要約」選択かつ未実行の状態ではリトライボタンが disabled になること
- [ ] 「要約」選択かつ未実行の状態で「⚠ 要約が選択されていますが未実行です〜」が表示されること
- [ ] 「選択した要約を実行」ボタンを押すと要約が実行され、「⟳ 要約中...」が表示されること
- [ ] 要約完了後、トークン削減率（例: `~5,200 トークン 65%削減`）が表示されること
- [ ] 要約完了後、「要約結果を表示」アコーディオンで要約テキストをプレビューできること
- [ ] 設計書のみ「要約」を選択してリトライし、レビューが正常に完了すること
- [ ] コードのみ「要約」を選択してリトライし、レビューが正常に完了すること
- [ ] 両方「要約」を選択してリトライし、レビューが正常に完了すること
- [ ] 「そのまま」（全てoriginal）の状態でリトライし、従来通り動作すること
- [ ] 「スキップ」が従来通り動作すること
- [ ] 要約版でレビュー完了後、StepCardに「要約版の設計書でレビューを実施しました」等の表記が出ること
- [ ] 要約版でレビュー完了後、StepCardに「使用した設計書の要約を表示」アコーディオンが出ること

#### F. 施策5: Phase 3（結果統合）の要約リトライUI

- [ ] 結果統合でエラー発生時、エラー内容がそのまま表示されること
- [ ] エラー発生時、エラー種別に関わらず各グループのレビュー結果ごとに「そのまま/要約」のラジオボタンが表示されること
- [ ] エラー発生時、注意文が表示されること
- [ ] 各グループの「そのまま」ラジオボタンに推定トークン数が表示されること
- [ ] 一部のグループのみ「要約」を選択して要約を実行できること
- [ ] 要約完了後、トークン削減率と「要約結果を表示」アコーディオンが表示されること
- [ ] 全ての「要約」選択グループの要約が完了するとリトライボタンが active になること
- [ ] 要約済みレビュー結果で統合リトライが正常に完了すること
- [ ] 追加で別のグループも「要約」に変更した場合、新規のみ再要約されること（既に要約済みのものは再実行されない）

#### G. 回帰テスト

- [ ] 一括レビューが従来通り動作すること（設計書+コードを読み込み、レビュー実行、結果表示）
- [ ] 分割レビューの正常フロー（エラーなし）が従来通り動作すること（構造マッチング→グループレビュー→結果統合→結果表示）
- [ ] 構造マッチングのリトライが従来通り動作すること
- [ ] グループレビューのリトライ（要約なし）が従来通り動作すること
- [ ] グループレビューのスキップが従来通り動作すること
- [ ] 結果統合のリトライ（要約なし）が従来通り動作すること
