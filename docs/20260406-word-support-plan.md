...
`mammoth`（Wordを HTMLに変換するライブラリ）と `lxml` が追加インストールされる。

#### `versions/v0.9.7/backend/app/routers/convert.py`

- エンドポイント `POST /excel-to-markdown` の拡張子チェックに `docx` を追加
- ドキュメント文字列・エラーメッセージを更新
...
if ext not in ["xlsx", "xls", "docx"]:
    ...error="対応していないファイル形式です。Excel (.xlsx, .xls) またはWord (.docx) ファイルを選択してください。"
```
