...
- [x] バックエンド: `GET /api/health` 追加、`GET /health` とロジック共通化
- [x] フロントエンド: `fetchHealth()` 関数追加（3パターンの戻り値: ok / http_error / network_error）
- [x] フロントエンド: マウント時バージョン比較、不一致バナー表示（http_error時も「バージョン確認API未対応」として警告）
- [x] ドキュメント: `versions/v0.9.5/spec.md` に `GET /api/health` と不一致検知機能を追記
- [x] バックエンドテスト追加・全テスト通過（189件）
- [x] フロントエンドテスト追加・全テスト通過（202件）
- [x] 手動動作確認（v0.9.1 バックエンド + v0.9.5 フロントエンドで警告バナー表示）
