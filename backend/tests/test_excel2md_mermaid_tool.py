...
from app.markdown_tools.excel2md_mermaid_tool import Excel2mdMermaidTool


class TestExcel2mdMermaidToolProperties:
    """Excel2mdMermaidToolのプロパティテスト。"""

    def test_name_property(self):
        """nameプロパティが正しい値を返す。"""
        tool = Excel2mdMermaidTool()

        assert tool.name == "excel2md_mermaid"

    def test_display_name_property(self):
        """display_nameプロパティが正しい値を返す。"""
        tool = Excel2mdMermaidTool()

        assert tool.display_name == "excel2md (CSV+Mermaid)"


class TestExcel2mdMermaidToolRegistry:
    """Excel2mdMermaidToolのレジストリ統合テスト。"""
    ...
    def test_tool_registered_in_registry(self):
        ...
        assert "excel2md_mermaid" in tool_names

    def test_get_tool_by_name(self):
        """get_markdown_tool("excel2md_mermaid")でツールを取得できる。"""
        tool = get_markdown_tool("excel2md_mermaid")

        assert isinstance(tool, Excel2mdMermaidTool)

    def test_case_insensitive_access(self):
        """大文字小文字を区別しないアクセス。"""
        tool_lower = get_markdown_tool("excel2md_mermaid")
        tool_upper = get_markdown_tool("EXCEL2MD_MERMAID")
        tool_mixed = get_markdown_tool("Excel2md_Mermaid")

        assert isinstance(tool_lower, Excel2mdMermaidTool)
        assert isinstance(tool_upper, Excel2mdMermaidTool)
        assert isinstance(tool_mixed, Excel2mdMermaidTool)


class TestExcel2mdMermaidToolConvert:
    """Excel2mdMermaidTool.convert() メソッドのテスト。"""

    @pytest.fixture
    def tool(self):
        """Excel2mdMermaidToolインスタンスを提供。"""
        return Excel2mdMermaidTool()
    ...
    def test_mermaid_option_enabled(self, tool, sample_xlsx_content):
        """Mermaidオプションが有効になっている（エラーにならない）。"""
        # シェイプがないExcelでもMermaidオプション有効でエラーにならないことを確認
        result = tool.convert(sample_xlsx_content, "test.xlsx")
