...
同一入力を2回実行する特性を活かし、LLMプロバイダーのプロンプトキャッシュを利用して2回目の入力コストを削減する。

| プロバイダー | 方式 | 備考 |
|------|------|------|
| Anthropic | `system`・ユーザーメッセージ末尾に `cache_control: {"type": "ephemeral"}` を付与 | キャッシュ読み取りは通常入力の約1/10のコスト |
| Bedrock | Converse API の `system`・`content` 末尾に `cachePoint: {"type": "default"}` を付与 | キャッシュ非対応モデルは `ValidationException` を検知して cachePoint なしで自動再試行 |
| OpenAI | 自動キャッシュ（リクエスト側の変更なし） | 1,024トークン以上のプレフィックスが対象 |

- キャッシュはプレフィックス一致のため、1回目と2回目のプロンプトはバイト単位で同一であること（現行実装は同一）
- モデルごとの最小キャッシュ対象トークン数（1,024〜4,096）未満の入力ではキャッシュされないが、エラーにはならない
- レスポンスの `inputTokens` はキャッシュ読み書き分を合算した総入力トークン数を返す（レビュー情報のトークン表示の一貫性を維持）
- 各プロバイダーはキャッシュの読み書きトークン数をサーバーログ（INFO）に出力する
...
```
┌─────────────────────────────────────────────────────────────┐
│  ( v0.8.3 )       設計書-Javaプログラム突合 AIレビュアー     │
│                                    📖 プリセット  [⚙️]      │
└─────────────────────────────────────────────────────────────┘
```
...
- CORS設定により、許可されたオリジンからのみアクセス可能
  - `CORS_ORIGINS` 未設定時はローカル開発用オリジンのみ許可（v0.8.3で全許可デフォルトを廃止）
  - `CORS_ORIGINS=*` を明示指定した場合は credentials を無効化する
- 入力ファイルのバリデーション（拡張子、MIMEタイプ、サイズ）
- アップロードファイル名はパス要素を除去して使用（パストラバーサル防止、v0.8.3）
- 予期しないサーバー内部エラーの詳細はAPIレスポンスに含めず、サーバーログにのみ出力（v0.8.3）
...
| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-VS-001 | バージョンボタン表示 | サーバー起動済み | ページにアクセス | 左上にピル型の「v0.8.3」ボタンが表示される | |
| E2E-VS-002 | バージョン切替 | v0.8.3表示中 | バルーンからv0.5.2をクリック | トースト表示後、ページリロード、v0.5.2のUIが表示される | |
| E2E-VS-003 | Cookie永続化確認 | E2E-VS-002完了 | ブラウザを閉じて再アクセス | v0.5.2が表示される（Cookieが保持されている） | |
| E2E-VS-004 | 無効なCookie時のフォールバック | - | DevToolsでCookieを「v9.9.9」に設定してアクセス | v0.8.3（デフォルト）が表示される | |
