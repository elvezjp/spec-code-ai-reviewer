...
```
┌─────────────────────────────────────────────────────────────┐
│  ( v0.8.1 )       設計書-Javaプログラム突合 AIレビュアー     │
│                                    📖 プリセット  [⚙️]      │
└─────────────────────────────────────────────────────────────┘
```
...
| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-VS-001 | バージョンボタン表示 | サーバー起動済み | ページにアクセス | 左上にピル型の「v0.8.1」ボタンが表示される | |
| E2E-VS-002 | バージョン切替 | v0.8.1表示中 | バルーンからv0.5.2をクリック | トースト表示後、ページリロード、v0.5.2のUIが表示される | |
| E2E-VS-003 | Cookie永続化確認 | E2E-VS-002完了 | ブラウザを閉じて再アクセス | v0.5.2が表示される（Cookieが保持されている） | |
| E2E-VS-004 | 無効なCookie時のフォールバック | - | DevToolsでCookieを「v9.9.9」に設定してアクセス | v0.8.1（デフォルト）が表示される | |
