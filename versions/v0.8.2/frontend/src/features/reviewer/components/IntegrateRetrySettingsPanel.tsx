...
export function IntegrateRetrySettingsPanel({
  ...
  const hasPendingSummarize = completedGroups.some(
  ...
  const handleExecuteSummarize = async () => {
    ...
    const results = await Promise.all(
      targets.map(async (g) => {
        try {
          ...
          return {
            groupId: g.groupId,
            summarizedReport: response.success ? response.summarizedText : undefined,
    ...
    // Update summarize state (preserve existing entries)
    const updatedGroups = [...summarizeState.groups]
    for (const result of results) {
      const existingIndex = updatedGroups.findIndex((s) => s.groupId === result.groupId)
      const entry: IntegrateGroupSummarizeEntry = {
        groupId: result.groupId,
        mode: 'summarize',
        summarizedReport: result.summarizedReport,
  ...
  const handleModeChange = (groupId: string, mode: 'original' | 'summarize') => {
    setGroupModes((prev) => ({ ...prev, [groupId]: mode }))
  }
      ...
      {completedGroups.map((g) => {
        ...
        return (
          <div key={g.groupId} className="mb-3">
            <p className="text-xs font-medium text-gray-600 mb-1">{g.groupName}</p>
