...
設計書種別と注意事項は、設定ファイル（`verifier-config.md`）の `specTypes` セクションで管理される（詳細は「2.6 設定ファイル管理」を参照）。

| 管理方法 | 説明 |
|---------|------|
| 設定ファイル | `verifier-config.md` の `specTypes` セクションにテーブル形式で定義 |
...
- [複数ファイル対応拡張方針案](extension-plan.md) - 構造化方式によるスキーマ拡張、メイン設計書選択機能、トークン最適化など将来の拡張方針
- [FastAPI + htmx 採用検討](fastapi_htmx_mock_research.md) - モック開発における技術選定の調査メモ
...
| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-VS-001 | バージョンボタン表示 | サーバー起動済み | ページにアクセス | 左上にピル型の「v0.5.0」ボタンが表示される | |
| E2E-VS-002 | バージョン切替 | v0.5.0表示中 | バルーンからv0.4.0をクリック | トースト表示後、ページリロード、v0.4.0のUIが表示される | |
| E2E-VS-003 | Cookie永続化確認 | E2E-VS-002完了 | ブラウザを閉じて再アクセス | v0.4.0が表示される（Cookieが保持されている） | |
| E2E-VS-004 | 無効なCookie時のフォールバック | - | DevToolsでCookieを「v9.9.9」に設定してアクセス | v0.5.0（デフォルト）が表示される | |

#### 13.3.5 旧バージョン動作確認テスト

| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-OLD-001 | v0.4.0 画面表示確認 | v0.4.0に切替済み | 画面を確認 | v0.4.0固有のUI（設定ファイル機能あり、複数回実行なし）が表示される | |
| E2E-OLD-002 | v0.4.0 レビュー実行 | v0.4.0表示中、設計書・コード変換済み | レビュー実行ボタン押下 | レビューレポートが正常に表示される（タブ切り替えなし） | |
| E2E-OLD-003 | v0.3.0 画面表示確認 | v0.3.0に切替済み | 画面を確認 | v0.3.0固有のUI（種別JSON設定あり、設定ファイル機能なし）が表示される | |
| E2E-OLD-004 | v0.3.0 レビュー実行 | v0.3.0表示中、設計書・コード変換済み | レビュー実行ボタン押下 | レビューレポートが正常に表示される | |
| E2E-OLD-005 | v0.2.5 画面表示確認 | v0.2.5に切替済み | 画面を確認 | v0.2.5固有のUI（優先度ドロップダウンあり、excel2mdツール選択なし）が表示される | |
| E2E-OLD-006 | v0.2.5 レビュー実行 | v0.2.5表示中、設計書・コード変換済み | レビュー実行ボタン押下 | レビューレポートが正常に表示される | |
| E2E-OLD-007 | v0.1.1 画面表示確認 | v0.1.1に切替済み | 画面を確認 | v0.1.1固有のUI（優先度・種別設定なし、トークン数表示なし）が表示される | |
| E2E-OLD-008 | v0.1.1 レビュー実行 | v0.1.1表示中、設計書・コード変換済み | レビュー実行ボタン押下 | レビューレポートが正常に表示される | |

#### 13.3.6 設定ファイル管理テスト

| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-CFG-001 | 設定ファイルアップロード | 設定モーダル表示中 | LLMとspecTypes両方を含むverifier-config.mdをドラッグ&ドロップ | ファイル名と「・LLM設定を更新しました」「・設計書種別と注意事項を更新しました」が緑色で表示される | |
