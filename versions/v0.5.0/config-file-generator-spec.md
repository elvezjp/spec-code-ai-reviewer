...
現在はspec-code-verifier用の設定（`verifier-config.md`）を生成するスキーマが設定されている。
...
このファイルをspec-code-verifierにアップロードして利用する。
...
ただし、このツールは spec-code-verifier 専用ではなく、**汎用的な設定ファイルジェネレーター**として設計し、以下に対応できるようにする：
- 設定ファイル仕様のバージョンアップ
- 別アプリケーションへの転用
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  spec-code-verifier の設定ファイルを作成します。              │
