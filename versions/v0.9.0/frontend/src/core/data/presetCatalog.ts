...
- 確信度は以下の基準で判定してください：
  ...
  - 低: 関連性はあるが確証がない
- 実装箇所が特定できない項目は「未マッピング項目一覧」に記載してください
- 各設計書の冒頭に記載されている役割、種別、注意事項を考慮してください。
- メイン以外の設計書は必要な場合に参照してください。
`,
    },
...
\`\`\`json
{
  "files": [
    {
      "designFile": "<API仕様書ファイル名>",
      "items": [
        {
          "designItem": "<HTTPメソッド パス（例: GET /api/users）>",
          "implementationElement": "<ハンドラー関数名（例: UserController#index）>",
          "implementationLocation": "<実装ファイル:行番号範囲>",
          "confidence": "<高|中|低>",
          "note": "<ミドルウェア情報等>"
        }
      ]
...
- 未実装エンドポイントはimplementationElement, implementationLocation, confidenceをすべて"-"としてください
- サマリーは不要です。itemsの内容から自動集計します`,
...
\`\`\`json
{
  "files": [
    {
      "designFile": "<DB設計書ファイル名>",
      "items": [
        {
          "designItem": "<テーブル名またはテーブル名.カラム名>",
          "implementationElement": "<モデル/エンティティクラス名>",
          "implementationLocation": "<実装ファイル:行番号範囲>",
          "confidence": "<高|中|低>",
          "note": "<リレーション情報等>"
        }
...
- 未実装のテーブル/カラムはimplementationElement, implementationLocation, confidenceをすべて"-"としてください
- サマリーは不要です。itemsの内容から自動集計します`,
