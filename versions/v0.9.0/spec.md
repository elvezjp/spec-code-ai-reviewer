...
v0.9.0では、設計書の各項目がソースコードのどこで実装されているかを特定する「マッピングモード」を追加。
トレーサビリティの可視化を実現する。
...
| モード | 目的 | 出力 |
|--------|------|------|
| 突合モード | 設計書とコードの整合性検証（差異・問題点の発見） | レビューレポート |
| マッピングモード | 設計書項目と実装箇所の対応付け | マッピング表 + カバレッジレポート |

#### 1.1.2 マッピングモードの特徴

- **モード切り替え**: 画面上部のタブで突合モード / マッピングモードを切り替え
- **マッピング用プリセット**: 標準マッピング、APIエンドポイントマッピング、データベーススキーママッピングの3種
- **構造マップ活用**: md2map/code2mapで生成したMAP.jsonをAIに提供し、マッピング精度を向上
- **カバレッジ判定**: マッピング結果のカバレッジ率（マッピング件数/設計書項目件数）による簡易判定

#### 1.1.3 useStructureMapオプション

構造マップ（MAP.json）のAI提供を制御するフラグ。
...
| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-OLD-001 | 旧バージョン画面表示確認 | 旧バージョンに切替済み | 画面を確認 | UI（単一HTMLファイル版、設定ファイル機能あり、複数回実行あり）が表示される | |
| E2E-OLD-002 | 旧バージョンレビュー実行 | 旧バージョン表示中、設計書・コード変換済み | レビュー実行ボタン押下 | レビューレポートが正常に表示される | |
