...
指示書: [`tests/review.md`](../../tests/review.md)
...
- 結果ファイル: `tests/results/YYYYMMDD-HHMM-review.md`
- 冒頭にサマリー（OK数 / NG数 / 全12項目）を出力
- NG の場合は該当箇所（ファイル名・行番号）と理由を備考に記載

#### 13.4.2 シナリオテスト

指示書: [`tests/scenarios.md`](../../tests/scenarios.md)

AI にセットアップから起動・疎通までのシナリオを順番に実行させ、各試験項目の OK / NG を判定させる。

##### シナリオ一覧

| シナリオ | 内容 | 試験項目数 |
|---------|------|-----------|
| 1. バックエンドのセットアップ | `uv sync` → `uv run pytest` | 2 |
| 2. フロントエンドのセットアップ | `npm ci` → `npm run test:run` → `npm run build` | 3 |
| 3. バックエンドの起動確認 | `uvicorn` 起動 → `/docs` アクセス | 2 |
| 4. API エンドポイントの疎通 | 変換API・分割APIのバリデーション確認 | 2 |
| 5. フロントエンドの起動確認 | `npm run dev` → ブラウザアクセス | 1 (+ 1 MANUAL) |

##### 結果報告

- 結果ファイル: `tests/results/YYYYMMDD-HHMM-scenario.md`
- 冒頭にサマリー（OK数 / NG数 / MANUAL数 / 全10項目）を出力
- MANUAL の項目はスキップし、結果に MANUAL と記載
- NG の場合はエラー内容や実際の出力を備考に記載
