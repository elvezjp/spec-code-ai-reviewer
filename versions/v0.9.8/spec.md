...
| トークン超過 | LLM APIからのエラー応答 | エラーコードに応じた表示 | 分割設定でのレビューを推奨 |
...
| ツール | 対象 | 分割単位 |
|--------|------|----------|
| [md2map](https://github.com/elvezjp/md2map) | Markdown設計書 | 見出し（H1-H6）単位 ※3つの分割モードから選択可能 |
| [code2map](https://github.com/elvezjp/code2map) | ソースコード | クラス/メソッド/関数単位 |

**md2mapの分割モード:**

md2mapは以下の3つの分割モードを提供する。ユーザーが分割レビュー選択時にモードを指定する。

| モード | 説明 | 特徴 |
|--------|------|------|
| 見出し（heading） | 見出し（H1-H6）の階層構造に基づいて分割する | デフォルト。高速で安定した分割。見出しが適切に構造化された文書に最適 |
| NLP | 自然言語処理（形態素解析）を用いて意味的な区切りで分割する | 見出しのない長大なセクションも適切な粒度で分割可能。LLM不要 |
| AI | LLM（大規模言語モデル）を用いて意味的な区切りで分割する | 最も高精度な分割が可能。LLM設定（プロバイダー・APIキー等）が必要 |

- NLP / AIモードでは、見出しによる基本分割に加え、見出しのない長大なセクションをサブスプリット（subsplit）として細分化する
- AIモードではレビュー実行に使用するLLM設定と同じ設定を使用する

#### 2.7.2 レビュー方式

レビュー方式は「一括」と「分割」の2択とする。ユーザーが明示的に選択する（自動判定は行わない）。

| レビュー方式 | 説明 |
|--------|------|
| 一括（従来） | 従来通り全体を一度にレビュー |
| 分割 | 設計書とコードの両方を分割してレビュー |
...
- 「分割」選択時に `POST /api/split/headings` で設計書の見出し一覧（H2まで）を取得し、事前指定パネルに表示する。取得結果はキャッシュし、一括↔分割の切り替えでは再取得しない。設計書マークダウンが変更された場合はキャッシュをリセットし、次に「分割」を選んだ時に再取得する。
- 事前指定パネルでは、セクション単位で「事前重要指定」または「事前除外指定」を選択できる。同一セクションに両方を指定することはできない（排他制御）。
...
レビュー方式で「分割」を選択すると、設計書のH2見出し一覧が事前指定パネルに表示される。各見出しには「事前重要指定」と「事前除外指定」のチェックボックスが配置される。

- **事前重要指定**: 重要なセクションに個別の分割設定を適用する
- **事前除外指定**: 不要なセクションを分割・レビューの対象から除外する（変更履歴、目次など）
- **排他制御**: 同一セクションに事前重要指定と事前除外指定の両方を指定することはできない。一方をONにすると他方は自動的にOFFになる

| 表示項目 | 説明 |
|----------|------|
| 事前重要指定 | チェックボックス。ONにしたセクションは事前重要指定セクションとして扱われる |
| 事前除外指定 | チェックボックス。ONにしたセクションは分割・レビューの対象から除外される |
| セクション名 | H2見出しテキスト |
| 行範囲 | 開始行-終了行 |
| 推定文字数 | セクションの推定文字数 |

事前除外指定セクションは md2map の `skip` 機能により分割処理の対象から完全に除外され、分割プレビュー結果に含まれない。これにより処理時間の短縮とトークン節約が実現される。
...
| 要約 | チェックボックス。チェックしたセクションはレビュー時に要約テキストで代替される |
...
事前除外指定セクションは分割処理の対象から除外されるため、分割プレビュー結果には含まれない。パート数にもカウントされない。

**コードパーツ:**

| 項目 | 説明 |
|------|------|
| 重要 | チェックボックス。重要指定されたコードシンボルは全グループに共通注入される |
| 要約 | チェックボックス。要約対象として選択し、「要約実行」で事前要約を実行する |
| 除外 | チェックボックス。レビュー対象から除外する。除外時は重要・要約が自動解除される |
| シンボル名 | クラス名、関数名など |
| 種別 | class / method / function |
| 行範囲 | 開始行-終了行 |
| 推定トークン | パーツの推定トークン数（要約選択時は要約後のトークン数を表示） |

各コードシンボルは「内容を表示」で展開表示でき、要約実行後は「要約結果を表示」で要約内容を確認できる。
コードパートの除外・重要指定・要約は設計書パートと同じ操作性で提供される。
大規模クラス（20,000行超など）がクラス全体のシンボルとして生成される場合、メソッドが個別シンボルとして存在するため、クラスシンボルを除外してメソッド単位でレビューすることでトークン上限エラーを回避できる。

#### 2.7.6 セクション除外機能

分割プレビューで設計書パートおよびコードパートをレビュー対象から除外する機能。
...
**目的:**
- 重要パート（全グループで参照されるセクション）が長大な場合、全グループに展開されるとトークン消費が大きくなるため、要約で削減する
- 要約は任意のパートに適用可能（重要パートに限らない）

**要約フロー:**
1. 分割プレビューの設計書パーツテーブルまたはコードパーツテーブルで、各パートの「要約」チェックボックスを選択
2. プレビュー結果末尾の「選択した要約を実行」ボタンをクリック（設計書・コード共通の統合ボタン）
3. 設計書→コードの順に、要約対象パートが1パートずつ逐次実行され、完了するたびにテーブルに結果が反映
4. エラー発生時はそのパートで中断し、成功済みパートの結果は保持
5. 全パート要約完了後、レビュー実行が可能になる
...
| 要約選択・未実行 | 「未実行」 |
| 要約実行中 | 「⟳ 要約中」 |
| 要約選択・実行済み | 要約後の推定トークン数 |

**要約結果の保持:**
- 要約済みパートのチェックを外しても要約テキストは内部に保持される
- 再度チェックすると、再要約なしで要約後トークン数が表示される

**レビュー実行時の動作:**
- 設計書またはコードで「要約」選択かつ未実行のパートがある場合、レビュー実行ボタンは disabled となり、警告メッセージが表示される
- 要約実行ボタンは設計書・コードで統合された1つのボタンとして、プレビュー結果の末尾に配置される。設計書→コードの順に逐次実行される
- グループレビューの documentContent 構築時に、要約済みパートは要約テキストで代替される
- 要約版のヘッダーには `[要約版]` が付与される

**要約API:**
- 設計書の要約は `/api/summarize` エンドポイントを使用（`targetType: "design"`）
- コードの要約は同エンドポイントを使用（`targetType: "code"`）
- 要約プロンプトは仕様の具体的な値・条件分岐・エラーケース等を保持し、書式説明や冗長な説明を省略する方針
...
- リトライ時にはリトライ設定パネルが表示され、設計書・コードの要約有無を選択してリトライ可能

**一括レビューのエラー時:**
- エラー発生時にリトライボタンを表示
- トークン上限エラーの場合は、分割設定でのレビューを推奨するメッセージを表示

**統合レビュー（フェーズ3）のエラー時:**
- リトライ設定パネルで各グループのレビュー結果について「そのまま」「要約」「スキップ」の3択を選択可能
- 要約は1グループずつ逐次実行され、完了後にリトライ実行
- スキップを選択したグループは統合対象から除外され、最終レポートに含まれない
- 全グループをスキップにすることはできない（最低1グループは統合対象に残す必要がある）
- スキップしたグループのレビュー結果は一式ダウンロード（ZIP）には含まれる
...
2. **設計書セクション**
   ...
   - **Markdown整理機能**（変換完了後に表示）
     ...
     - 説明テキスト: 「AIを使用してMarkdownの構造を整理します。原文の意味は変更されません。トークン数が多い場合は処理に時間がかかったり、エラーになる可能性があります。また、一部の情報や制約が失われることがあります。分割設定でのレビューもお試しください。」
...
6. **分割設定セクション**
   ...
   - レビュー方式: 一括 / 分割（md2map と code2map の仕様に準拠し、解析と分割を行う）
   - 分割選択時は設計書とコードの両方を分割する（片側だけの分割は不可）
   - 設計書またはプログラムが未選択の場合、分割は選択できない（ラジオボタン無効）
   - 未対応言語のコードファイルが含まれる場合、分割は選択できない（トースト通知）
   - 分割オプション（分割選択時に展開）
     - 事前指定パネル: 設計書のH2見出し一覧を表示し、セクション単位で「事前重要指定」「事前除外指定」を選択可能（排他制御あり）
     - 設計書分割モード: 見出し（デフォルト） / NLP / AI の3択（ラジオボタン）
       - AIモード選択時はLLM設定が必要である旨を注記表示
     - 設計書見出しレベル: H2まで（推奨）/ H3まで / H4まで
     - プログラム: 対応言語表示（Python / Java）、未対応ファイルの警告
   - 「分割プレビュー」ボタン（プレビュー実行後は「プレビュー再実行」に変化し、クリックで結果をクリアして再実行）
   - プレビュー完了時: ボタン横に「✓ 分割プレビュー完了」と「設定を変更した場合は再実行してください。」を表示
   - プレビュー結果表示（設計書パーツ一覧、コードパーツ一覧）

7. **レビュー実行セクション**
   - 分割選択時は「分割プレビュー」が必須（未実行時はレビュー実行ボタン無効）
   - 一括モード時: 「同じ設定でレビューを2回実行します。それぞれ個別に結果を確認できます。」
   - 分割モード時: 「設計書とプログラムの関連を分析して、いくつかのグループに分割してレビューを実行します。」
...
```
┌─────────────────────────────────────────────────────────────┐
│  ( v0.9.1 )       設計書-Javaプログラム突合 AIレビュアー     │
│                                    📖 プリセット  [⚙️]      │
└─────────────────────────────────────────────────────────────┘
```
...
| token_limit | LLM APIからのトークン数超過エラー |
| api_error | APIエラー（5xx等） |
| format_invalid | 出力形式が不正 |
| policy_empty | 整理方針が空 |

**備考:** 入力側のトークン上限チェックは行わない。入力全体をそのままLLMに渡し、LLM API側でトークン超過が発生した場合は `token_limit` エラーとして返却する。
...
`pre_excluded_sections` が指定された場合、該当セクションは md2map の `section_overrides` に `skip: true` として渡され、分割結果に含まれない。`pre_important_sections` と `pre_excluded_sections` に同一の `start_line` を指定することはできない（排他制御はフロントエンドで実施）。

**splitMode の詳細:**

| 値 | 説明 |
|-----|------|
| `heading` | 見出し（H1-H6）の階層構造に基づく分割（デフォルト） |
| `nlp` | 自然言語処理（形態素解析）による意味的分割。LLM不要 |
...
- `doc_sections` または `code_symbols` が空のグループは、フロントエンドで自動スキップされる（API呼び出しなし）。これは設計にあるがコード未実装、または設計書に記載のない追加コード等の正当なケースに対応する。画面にはスキップ理由が明示される。
...
テキスト要約API。設計書、コード、レビュー結果を要約する。事前要約機能やリトライ時の要約で使用される。
...
| `targetType` | `string` | ○ | `"design"` / `"code"` / `"review_result"` |
| `llmConfig` | `LLMConfig \| null` | × | LLM設定。未指定時はシステムLLMを使用 |

**targetType別の要約方針:**

| targetType | 保持する情報 | 省略してよい情報 |
|-----------|-------------|----------------|
| `design` | 仕様の具体的な値、条件分岐、エラーケース、制約、入出力の項目名と型、元の行番号参照 | 書式設定の説明、同じ内容の繰り返し、例の詳細展開 |
| `code` | 関数/メソッドのシグネチャ、バリデーション条件、条件分岐、エラーハンドリング、ビジネスロジック、行番号コメント | import文、getter/setter、ログ出力、自明なコメント |
| `review_result` | 全ての指摘事項、重要度、対応推奨事項、申し送り事項 | 「問題なし」の詳細、背景説明、重複指摘 |
...
{
  "success": true,
  "summarizedText": "要約されたテキスト",
  "originalTokens": 4500,
  "summarizedTokens": 1500,
  "tokensUsed": {"input": 5000, "output": 1500},
  "error": null
}
```
...
| `summarizedText` | `string \| null` | 要約後テキスト |
| `originalTokens` | `int \| null` | 元テキストの推定トークン数 |
| `summarizedTokens` | `int \| null` | 要約後の推定トークン数 |
| `tokensUsed` | `object \| null` | LLM消費トークン `{"input": N, "output": M}` |
...
| 種別 | 目的 | 実行方法 |
|------|------|---------|
| CI（マルチOS） | 複数OS・複数バージョンでのセットアップとテスト実行の検証 | GitHub Actions（自動） |
| バックエンド単体テスト | 各モジュールの関数レベルの動作検証 | pytest（自動） |
| フロントエンド単体テスト | カスタムフック・ユーティリティの動作検証 | Vitest（自動） |
| AIレビュー | コードの安全性・整合性の静的レビュー | AI実行（リリース前） |
| AIシナリオテスト | セットアップ・起動・疎通の結合テスト | AI実行（リリース前） |
...
`.github/workflows/ci.yml` で定義。`main` ブランチへの push / pull_request をトリガーに、マルチOS × マルチバージョンのマトリクスで依存関係のインストールと単体テストを自動実行する。
...
リリース前に AI（Claude Code 等）を用いてレビューとシナリオテストを実施する。指示書と結果はリポジトリルートの `tests/` ディレクトリで管理する。

| ファイル | 内容 |
|---------|------|
| `tests/review.md` | AIレビュー指示書（共通観点 + プロジェクト固有の観点） |
| `tests/scenarios.md` | シナリオテスト指示書（セットアップ・起動・疎通） |
| `tests/results/` | 実行結果の保存先（`YYYYMMDD-HHMM-{review\|scenario}.md`） |
...
| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-VS-001 | バージョンボタン表示 | サーバー起動済み | ページにアクセス | 左上にピル型の「v0.9.1」ボタンが表示される | |
| E2E-VS-002 | バージョン切替 | v0.9.1表示中 | バルーンからv0.5.2をクリック | トースト表示後、ページリロード、v0.5.2のUIが表示される | |
| E2E-VS-003 | Cookie永続化確認 | E2E-VS-002完了 | ブラウザを閉じて再アクセス | v0.5.2が表示される（Cookieが保持されている） | |
| E2E-VS-004 | 無効なCookie時のフォールバック | - | DevToolsでCookieを「v9.9.9」に設定してアクセス | v0.9.1（デフォルト）が表示される | |

#### 13.3.5 旧バージョン動作確認テスト

| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-OLD-001 | v0.8.0 画面表示確認 | v0.8.0に切替済み | 画面を確認 | v0.8.0固有のUI（単一HTMLファイル版、設定ファイル機能あり、複数回実行あり）が表示される | |
| E2E-OLD-002 | v0.8.0 レビュー実行 | v0.8.0表示中、設計書・コード変換済み | レビュー実行ボタン押下 | レビューレポートが正常に表示される | |
...
### 13.4 結合テスト（AIレビュー・シナリオテスト）

リリース前に AI を用いて実施する結合テスト。指示書の内容を AI に渡し、結果を `tests/results/` に保存する。
...
- 結果ファイル: `docs/tests/results/YYYYMMDD-HHMM-review.md`
- 冒頭にサマリー（OK数 / NG数 / 全12項目）を出力
- NG の場合は該当箇所（ファイル名・行番号）と理由を備考に記載
...
| シナリオ | 内容 | 試験項目数 |
|---------|------|-----------|
| 1. 画面表示の確認 | トップページ・各セクションの描画確認 | 5 |
| 2. 設計書の変換 | Excel 選択 → ツール選択 → Markdown 変換 | 2 (+ 2 MANUAL) |
| 3. プログラムの変換 | コードファイル選択 → 行番号付与 | 2 (+ 1 MANUAL) |
| 4. レビュー実行 | レビュー実行 → レポート・簡易判定・レビュー情報の表示確認 | 5 |
| 5. レビュー結果のダウンロード | コピー・MD ダウンロード・ZIP 一式ダウンロードの確認 | 6 |
| 6. エラーハンドリング | コード未指定でのレビュー実行 | 1 |
| 7. 設定モーダル | モーダル表示・LLM 未設定表示・接続テスト・リンク遷移 | 4 |

##### 結果報告

- 結果ファイル: `docs/tests/results/YYYYMMDD-HHMM-scenario.md`
- 冒頭にサマリー（OK数 / NG数 / MANUAL数 / 全27項目）を出力
- MANUAL の項目はスキップし、結果に MANUAL と記載
- NG の場合はエラー内容やスクリーンショットの所見を備考に記載
