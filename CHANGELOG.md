...
- **Updated the frontend development dependency `vitest` from 4.1.9 to 4.1.11** to address path traversal / arbitrary file read via the `@vitest/mocker` redirect mock (GHSA-82fw-gwwq-j7x9, Dependabot [#982](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/982) / [#981](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/981)). Updated the current version's lockfile, covering nine packages including `@vitest/mocker`.
- **Updated the frontend development dependency `browserslist` from 4.28.2 to 4.28.9** to address crashes and prototype writes through untrusted custom statistics JSON (GHSA-73wf-gq98-2v4g, Dependabot [#979](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/979)). Updated the current version's lockfile, including five related dependencies.
- **Updated the frontend development dependency `js-yaml` from 4.3.0 to 4.3.2** to address excessive CPU consumption when resolving `!!omap` (GHSA-5p4m-2wfm-xmqj, Dependabot [#976](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/976)). Updated the current version's lockfile.
- **Fixed a path traversal in unauthenticated APIs that allowed arbitrary file writes** (GHSA-97h8-c8cg-hwm2): `POST /api/split/markdown`, `POST /api/split/code`, and `POST /api/convert/excel-to-markdown` joined the client-supplied filename directly into a temporary directory path, so absolute paths or `../` sequences could create or overwrite files outside the temporary directory. Client-supplied filenames are now sanitized with `safe_filename()` (added in `backend/app/safe_path.py`), which strips directory components, with regression tests. A Windows-only failure in the added tests was fixed in the follow-up (#130)
- **Fixed an SSRF in unauthenticated APIs caused by an unvalidated `baseUrl`** (GHSA-hp5g-x4rg-8m95): The OpenAI-compatible endpoint `baseUrl`, supplied through `llmConfig`, was passed to the HTTP client without validation. Because the API is served without authentication, anyone able to reach it could use this server to reach internal services, or a cloud instance metadata endpoint (`169.254.169.254`) to read its credentials. `baseUrl` is now validated once on the model (`LLMConfig`) via `validate_llm_base_url()` (added in `backend/app/url_guard.py`): the scheme must be `http` or `https`, the hostname is resolved, and private, loopback, link-local, reserved, multicast, and unspecified addresses are rejected; hostnames that fail to resolve are rejected as well (fail closed). Regression tests added. **Note:** connecting to an LLM running locally now requires setting `LLM_ALLOW_PRIVATE_BASE_URL=1`. Do not enable it where untrusted callers can reach this API. See "Local LLM connections" in the README, and the commented-out entry in `.env.example`
- **Stopped allowing credentials when CORS origins are not restricted** (#133): With `CORS_ORIGINS` at its default of `*`, `allow_credentials=True` was set. Starlette cannot combine a wildcard with credentials, so in that case it echoes the request's `Origin` back in `Access-Control-Allow-Origin`. Any site could therefore reach this API with credentials and read the response, so a malicious page opened while the app was running locally could read the code and design documents under review. Credentials are now allowed only when origins are restricted; behavior with explicit origins is unchanged. Regression tests added
- **Stopped printing LLM responses to standard output** (#133): The structure-matching LLM response was printed unconditionally in `review.py`. Responses contain the design documents and code under review, so they were exposed to anyone able to read standard output and to any platform collecting container logs. Now emitted with `logger.debug()` and not written by default
- **Pinned git dependencies to release tags instead of a mutable branch** (#133, CWE-829): `[tool.uv.sources]` referenced `branch = "main"` for `add-line-numbers`, `md2map`, and `code2map`, so the resolved commit moved whenever `main` advanced and a change of reference target could go unnoticed. The references are now the tags `v0.1.3` / `v0.5.1` / `v0.3.0`, so re-locking no longer moves the target and updating it becomes a reviewable change to `pyproject.toml`. Version floors are also declared in `[project.dependencies]`, which — unlike `[tool.uv.sources]` — propagate to downstream consumers through the built wheel's `Requires-Dist`. None of the three releases changes the implementation or the output of any command
- **Restricted the default CORS origins** (#117): `CORS_ORIGINS` defaulted to `*`, so with no authentication on any endpoint, any site could call this API from a browser and read the response — including the code and design documents under review. When unset, only the Vite development server and preview origins are now allowed (`http://localhost:5173`, `http://127.0.0.1:5173`, `http://localhost:4173`, `http://127.0.0.1:4173`); an empty or whitespace-only value is treated the same way rather than falling back to allow-all. `*` still works when given explicitly, with credentials disabled as before. **Note:** a normal setup is unaffected, because the frontend always calls the API from a single origin — the Vite proxy forwards `/api` server-side in development, and the same FastAPI app serves the frontend in production. Set `CORS_ORIGINS` explicitly if you run the frontend on a different host or call the API directly from another origin. `CORS_ORIGINS` is now documented in the READMEs and `.env.example`
- **Documented the absence of authentication as a deliberate decision** (#134): The application has no authentication or authorization on any endpoint, and this is now stated as a decision rather than an omission — option A in #134, "run locally, do not expose on a network". Added an "Intended Environment" section to SECURITY (English and Japanese) covering what is not guaranteed (unlimited LLM calls, and read access to the uploaded documents, code, and review results), what operators are asked to do (bind to `127.0.0.1`, require authentication at a reverse proxy when exposing the service, do not widen the CORS allowlist), and the background — up to v0.9.9 the bundled nginx configuration performed Basic authentication, and it was removed with the rest of the multi-version infrastructure in #118. Added a matching section to the READMEs, and listed the missing authentication under Known Security Limitations (CWE-306)
- **Documented an explicit bind address in the startup command** (#134): The README's backend startup command omitted `--host` and relied on the uvicorn default, leaving the service exposed if that default changed or if a deployment used `0.0.0.0`. It now specifies `--host 127.0.0.1`, with a note that every endpoint is reachable without authentication when bound to a public interface. This is a documentation change; no code behavior changed
- **Updated frontend dependencies to resolve Dependabot alerts** (#131): Bumped `react-router-dom` 7.17.0 → 7.18.2 to resolve alerts [#966](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/966) / [#968](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/968) / [#969](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/969) / [#970](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/970) (open redirect, route-matching DoS, XSS, constructor injection), and updated the transitive dev dependencies `js-yaml` → 4.3.0 (alert [#965](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/965)) and `brace-expansion` → 1.1.16 / 5.0.8 (alerts [#963](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/963) / [#964](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/964)), both CPU-consumption DoS. Alert [#967](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/967) (GHSA-qwww-vcr4-c8h2, RSC-mode CSRF) was dismissed as not applicable: the unstable RSC APIs are not used in this app and no 7.x patch exists
- **Updated `postcss` to resolve Dependabot alert [#971](https://github.com/elvezjp/spec-code-ai-reviewer/security/dependabot/971)** (#132): Bumped the transitive dev dependency `postcss` (via `vite`) 8.5.15 → 8.5.24 to address GHSA-r28c-9q8g-f849 (arbitrary `.map` file disclosure via source-map auto-loading)
