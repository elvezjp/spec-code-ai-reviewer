...
- **Aligned the Node.js requirement with the dependencies** (#140): 20.19+ (20.x) / 22.12+ (22.x) / 24+. The frontend CI Node.js matrix also changed from `["20", "23"]` to `["20", "24"]`
...
### Security
- **Fixed a path traversal in unauthenticated APIs that allowed arbitrary file writes** (GHSA-97h8-c8cg-hwm2): `POST /api/split/markdown`, `POST /api/split/code`, and `POST /api/convert/excel-to-markdown` joined the client-supplied filename directly into a temporary directory path, so absolute paths or `../` sequences could create or overwrite files outside it. Filenames are now sanitized with `safe_filename()`, which strips directory components, with regression tests (a test failure on Windows was fixed in #130)
- **Fixed an SSRF caused by an unvalidated `baseUrl` in unauthenticated APIs** (GHSA-hp5g-x4rg-8m95): The `baseUrl` in `llmConfig` was passed to the HTTP client without validation, so this server could be used as a stepping stone to reach internal networks or the cloud instance metadata endpoint (`169.254.169.254`). `validate_llm_base_url()` now restricts the scheme to `http` / `https`, resolves the host, and rejects private, loopback, link-local, and similar addresses as well as hosts that cannot be resolved (fail closed). **Note:** connecting to an LLM running locally now requires `LLM_ALLOW_PRIVATE_BASE_URL=1`. Do not enable it where untrusted parties can call this API (see "Local LLM connections" in the README)
- **[BREAKING] Changed the CORS default from allow-all (`*`) to local development origins only** (#117, #133, #140): When `CORS_ORIGINS` is unset or blank, only `http://localhost:5173` / `http://127.0.0.1:5173` / `http://localhost:4173` / `http://127.0.0.1:4173` are allowed. Credentials (`allow_credentials`) are never allowed when the setting contains `*`, including mixed values such as `https://app.example.com,*`. Previously the allow-all default combined with `allow_credentials=True` made Starlette echo the requesting Origin, so any site could read responses containing the code and design documents under review. **Note:** the frontend calls the API from the same origin, so typical setups are unaffected; if you call the API from a different origin, set `CORS_ORIGINS` explicitly
- **Stopped printing LLM responses to standard output** (#133): The structure-matching LLM response, which contains the design documents and code under review, was always printed, exposing it to anyone who can read standard output or container logs. It now goes through `logger.debug()` and is not emitted by default
- **Pinned git dependencies to release tags instead of a mutable branch** (#133, CWE-829): `add-line-numbers` / `md2map` / `code2map` now reference the tags `v0.1.3` / `v0.5.1` / `v0.3.0` instead of `branch = "main"`, and version floors were declared. None of the three releases changes implementation or output
- **Documented that the tool is intended for local use** (#134, #140): The absence of authentication and authorization on all endpoints (CWE-306) is now stated as a design decision rather than an omission. README / SECURITY gained an "Intended environment" section and the launch command now specifies `--host 127.0.0.1`. SECURITY now supports the latest version (0.10.0) only, and vulnerability reports go through private channels only (GitHub private vulnerability reporting or email)
- **Updated frontend dependencies to address known vulnerabilities** (#131, #132): `react-router-dom` 7.17.0 → 7.18.2 (open redirect, route-matching DoS, XSS, constructor injection), `vitest` 4.1.9 → 4.1.11 (GHSA-82fw-gwwq-j7x9), `browserslist` 4.28.2 → 4.28.9 (GHSA-73wf-gq98-2v4g), `js-yaml` → 4.3.2 (GHSA-5p4m-2wfm-xmqj and others), `brace-expansion` → 1.1.16 / 5.0.8, `postcss` 8.5.15 → 8.5.24 (GHSA-r28c-9q8g-f849). GHSA-qwww-vcr4-c8h2 (CSRF in `react-router` RSC mode) was dismissed: the affected feature is not used and no 7.x fix exists
...
|---------|-------------|
| 0.10.0  | Base URL / Reasoning Effort settings for OpenAI-compatible APIs, concurrent AI calls in split preview, retired `versions/` and moved to a root layout, version-switching feature removed, security fixes (path traversal, SSRF, CORS default restricted to local origins) |
